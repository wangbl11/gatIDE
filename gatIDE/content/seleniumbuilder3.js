...
sebuilder.boot = function() {
  ...
  if (sebuilder.getRecordingWindow().location.href.substring(0, "about:".length) == "about:") {
    sebuilder.getRecordingWindow().location = "http://www.baidu.com";
  }
...
function onCreated(windowInfo) {
  ...
  var tab0=windowInfo.tabs[0];
  console.log(`Created tab: ${tab0.id}`);
  browser.tabs.executeScript(tab0.id,{file: "/content/html/borderify.js"})
  .then(onScript)
  .catch(onError);
}

sebuilder.boot1=function() {
  console.log("injecting");
  var popupURL = browser.extension.getURL("content/html/gui.html");
  ...
  //sebuilder.recorderWindow = window.open("content/html/gui.html", "seleniumbuilder", "width=550,height=600,top=50,left=50,toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,copyhistory=no,resizable=yes");
  /*
...
builder = sebuilder;
browser.browserAction.onClicked.addListener(sebuilder.boot1);